...
function NavLinks({ onClick }: { onClick?: () => void }) {
  return (
    <>
      <Link to="/tools" className="font-serif whitespace-nowrap hover:underline" onClick={onClick}>
        Playground
      </Link>
      <Link to="/docs" className="font-serif whitespace-nowrap hover:underline" onClick={onClick}>
        Docs
      </Link>
    </>
  );
...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 z-50 w-full rounded-b-lg bg-transparent shadow-2xs backdrop-blur supports-backdrop-filter:bg-background/60">
      <div className="flex h-16 w-full items-center gap-2 px-4 sm:px-8">
        <div className="flex min-w-0 flex-1 items-center">
          <Link
            to="/"
            className="flex gap-1.5 font-mono text-lg tracking-tight whitespace-nowrap"
          >
            <TerminalIcon className="dark:stroke-primary" />
            Commandly
          </Link>
        </div>
        <div className="hidden min-w-0 flex-1 justify-center gap-8 md:flex">
          <NavLinks />
        </div>
